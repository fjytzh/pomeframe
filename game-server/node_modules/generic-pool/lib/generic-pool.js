...
exports.Pool = function (factory) {
   ...
   * 自动销毁连接池中的失效连接
   */
  function autoRemoveLoseConnect(){
    var connectAry = [];
    var i;
    var allcount = availableObjects.length;
    for(i=0;i<allcount;++i){
      if(availableObjects[i]['obj']["writable"] != false && availableObjects[i]['obj']["readable"] != false){
        connectAry.push(availableObjects[i]);
      }else{
        me.destroy(availableObjects[i].obj);
      }
    }
    availableObjects = connectAry;
  }
  ...
  function dispense() {
    ...
    if (waitingCount > 0) {
      //autoRemoveLoseConnect();
      while (availableObjects.length > 0) {
        log("dispense() - reusing obj", 'verbose');

        objWithTimeout = availableObjects[0];
        //确保DB出问题后恢复=>ok
        if(objWithTimeout.obj._socket.writable==false || objWithTimeout.obj._socket.readable==false){
            me.destroy(objWithTimeout.obj);
          availableObjects.unshift();
          continue;
        }
